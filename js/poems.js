 ...
 * 每個 segment 是一關；segment 內以 seg(句, 標點, 句, 標點, …) 交替排列，
 * 「句」一定不含標點，這樣遊戲只會要求玩家吃真正的字。
 */
(function (global) {
  const PUNCT = /[，。；：、？！「」『』（）《》〈〉·—…\s]/g;
  ...
  function seg() {
    ...
    for (let i = 0; i < arguments.length; i += 2) {
      lines.push(String(arguments[i]).replace(PUNCT, ""));
  ...
  function yueyangPart(id, part, lead, segments) {
    return {
      id: id,
      title: "岳陽樓記",
      part: part,
  ...
  /* 教育局《指定文言經典學習材料原文》全文，按段選玩、按句組入關。 */
  const yueyangP1 = yueyangPart("yueyang-p1", "第一段", "慶曆四年春，滕子京謫守巴陵郡……", [
    seg("慶曆四年春", "，", "滕子京謫守巴陵郡", "。"),
    seg("越明年", "，", "政通人和", "，", "百廢具興", "。"),
    seg("乃重修岳陽樓", "，", "增其舊制", "，"),
    seg("刻唐賢", "、", "今人詩賦於其上", "；"),
  ...
  const yueyangP2 = yueyangPart("yueyang-p2", "第二段", "予觀夫巴陵勝狀，在洞庭一湖……", [
    seg("予觀夫巴陵勝狀", "，", "在洞庭一湖", "。"),
    seg("銜遠山", "，", "吞長江", "，", "浩浩湯湯", "，", "橫無際涯", "；"),
    seg("朝暉夕陰", "，", "氣象萬千", "。"),
    seg("此則岳陽樓之大觀也", "，", "前人之述備矣", "。"),
    seg("然則北通巫峽", "，", "南極瀟湘", "，"),
    seg("遷客騷人", "，", "多會於此", "，"),
    seg("覽物之情", "，", "得無異乎", "？"),
  ]);

  const yueyangP3 = yueyangPart("yueyang-p3", "第三段", "若夫霪雨霏霏，連月不開……", [
    seg("若夫霪雨霏霏", "，", "連月不開", "；"),
    seg("陰風怒號", "，", "濁浪排空", "；"),
    seg("日星隱耀", "，", "山岳潛形", "；"),
    seg("商旅不行", "，", "檣傾楫摧", "；"),
  ...
  const yueyangP4 = yueyangPart("yueyang-p4", "第四段", "至若春和景明，波瀾不驚……", [
    seg("至若春和景明", "，", "波瀾不驚", "，"),
    seg("上下天光", "，", "一碧萬頃", "；"),
    seg("沙鷗翔集", "，", "錦鱗游泳", "，"),
    seg("岸芷汀蘭", "，", "郁郁青青", "。"),
  ...
  const yueyangP5 = yueyangPart("yueyang-p5", "第五段", "嗟夫！予嘗求古仁人之心……", [
    seg("嗟夫", "！", "予嘗求古仁人之心", "，"),
    seg("或異二者之為", "。", "何哉", "？"),
    seg("不以物喜", "，", "不以己悲", "。"),
    seg("居廟堂之高", "，", "則憂其民", "；"),
  ...
  const CURRICULUM = [
    ...
    { id: "c08", no: "捌", title: "始得西山宴遊記", subtitle: "柳宗元", works: [xishan] },
    {
      ...
      subtitle: "范仲淹",
      unit: "段",
      works: [yueyangP1, yueyangP2, yueyangP3, yueyangP4, yueyangP5],
    },
  ...
  function workLabel(work) {
    if (work.part) return "《" + work.title + "》" + work.part;
    const inner = work.title + (work.subtitle ? "・" + work.subtitle : "");
    const isVerse = ["詞", "五律", "七律", "五古"].indexOf(work.form) !== -1;
    return isVerse ? "〈" + inner + "〉" : "《" + inner + "》";
  }
